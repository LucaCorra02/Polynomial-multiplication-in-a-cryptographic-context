...
int* schoolbook_f9(int n, int* p1, int* p2) {
    int dim_ris = (2 * n) - 1;
    int* ris = calloc(dim_ris, sizeof(unsigned int));
    if (n == 1){
    ...
    int max_deg = n - 1;
    unsigned int mst_p1 = int_to_f9_element(p1[max_deg]);
    unsigned int mst_p2 = int_to_f9_element(p2[max_deg]);
    ris[max_deg * 2] = f9_sum(int_to_f3(ris[max_deg * 2]), f9_prod(mst_p1,mst_p2));
